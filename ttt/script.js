...
function selectWinner() { //if the one of following winning combination match then select the winner
    if (checkIdSign(1, 2, 3, playerSign) || checkIdSign(4, 5, 6, playerSign) || checkIdSign(7, 8, 9, playerSign) || checkIdSign(1, 4, 7, playerSign) || checkIdSign(2, 5, 8, playerSign) || checkIdSign(3, 6, 9, playerSign) || checkIdSign(1, 5, 9, playerSign) || checkIdSign(3, 5, 7, playerSign)) {
        ...
        wonText.innerHTML = `Jugador <p>${playerSign}</p> ¡ganó el juego!`; //displaying winning text with passing playerSign (X or O)
    } else { //si todas las casillas/elementos tienen valor de identificación y todavía nadie gana, empate el partido
        if (getIdVal(1) != "" && getIdVal(2) != "" && getIdVal(3) != "" && getIdVal(4) != "" && getIdVal(5) != "" && getIdVal(6) != "" && getIdVal(7) != "" && getIdVal(8) != "" && getIdVal(9) != "") {
            runBot = false; //pasar el valor booleano falso a runBot para que el bot no vuelva a ejecutarse
            bot(runBot); //llamando a la función de bot
            setTimeout(() => { //después del partido empatado, oculta el tablero y muestra el cuadro de resultados después de 700 ms
                resultBox.classList.add("show");
                playBoard.classList.remove("show");
            }, 700); //1s = 1000ms
            wonText.textContent = "¡El partido ha sido empatado!"; //mostrando texto de coincidencia de sorteo
        }
    }
}

replayBtn.onclick = () => {
    window.location.reload(); //volver a cargar la página actual al hacer clic en el botón de reproducción
}